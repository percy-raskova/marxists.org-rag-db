...
class RAGIngestor:
    ...
    def setup_chroma(self, persist_directory: Path):
        ...
        collection = client.get_or_create_collection(
            name="marxist_theory",
            metadata={"description": "Marxists Internet Archive corpus"}
        )
        
        print(f"✓ ChromaDB initialized at {persist_directory}")
        return collection
    
    def setup_qdrant(self, path: Optional[str] = None, url: Optional[str] = None):
...
Examples:
  ...
  python rag_ingest.py --db chroma --embedding-model mxbai-embed-large --markdown-dir ~/marxists-processed/markdown/
    ...
    parser.add_argument('--db', choices=['chroma', 'qdrant'], default='chroma',
    ...
    if args.db == "chroma":
        ...
        ingestor.db = ingestor.setup_qdrant(
            path=str(args.persist_dir) if not args.qdrant_url else None,
            url=args.qdrant_url
        )
