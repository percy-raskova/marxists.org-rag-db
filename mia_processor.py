...
try:
    ...
    print("Install with: pip install requests beautifulsoup4 markdownify pymupdf4llm --break-system-packages")
    exit(1)
...
class MIAProcessor:
    ...
    def html_to_markdown(self, html_path: Path) -> Optional[tuple[str, DocumentMetadata]]:
    ...
    def pdf_to_markdown(self, pdf_path: Path) -> Optional[tuple[str, DocumentMetadata]]:
...
Examples:
  ...
  python mia_processor.py --process-archive ~/Downloads/dump_www-marxists-org/ --output ~/my-rag-data/
    ...
    parser.add_argument('--download-json', action='store_true',
                        help='Download MIA JSON metadata files')
    parser.add_argument('--process-archive', type=Path,
                        help='Path to MIA archive directory')
    parser.add_argument('--output', type=Path, default=Path("~/marxists-processed").expanduser(),
